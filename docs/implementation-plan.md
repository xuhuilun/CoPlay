...
- Admin user management: users carry a `banned` flag with `GET /api/admin/users` and `POST /api/admin/users/:id/ban`. A ban revokes the user's authenticated identity — `/auth/me` resolves to null, the admin guard treats them as unauthenticated (401, even a self-banned admin), and the GitHub callback refuses a session (403) — rather than a spoofable submit block; abuse itself is bounded by the IP-keyed quota.
- Auth users and sessions are durable under `PERSISTENCE_DRIVER=prisma`: `PrismaUserStore`/`PrismaSessionStore` (MySQL) back the `UserStore`/`SessionStore` interfaces, so identities, bans, and login sessions survive restarts. Session ids are unguessable `randomUUID` tokens (not sequential defaults), and expired sessions are pruned on access. The in-memory stores remain the default for dev/single-instance. Adds `User`/`Session` models + migration; field mapping is unit-tested.
